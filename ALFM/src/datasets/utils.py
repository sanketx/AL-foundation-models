...
from torchvision.datasets.utils import download_and_extract_archive, verify_str_arg, download_url
...
class INaturalist2021(VisionDataset):
    ...
    def __init__(
        ...
        self.version = verify_str_arg(version, "version", INaturalist2021.DATASET_URLS.keys())
        ...
        if self.version[:4] == "2021":
            self.target_type = [verify_str_arg(t, "target_type", ("full", *self.CATEGORIES_2021)) for t in target_type]
            self._init_2021()
        else:
            raise RuntimeError(f"This iNaturalist split is not supported: {self.version}")
    ...
    def download(self) -> None:
        ...
        download_and_extract_archive(
            INaturalist2021.DATASET_URLS[self.version], base_root, filename=f"{self.version}.tgz", md5=INaturalist2021.DATASET_MD5[self.version]
        )

        orig_dir_name = os.path.join(base_root, os.path.basename(INaturalist2021.DATASET_URLS[self.version]).rstrip(".tar.gz"))
